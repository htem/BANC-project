...
def run_cascade_analysis(meta_df, edgelist_df, mask_A, mask_B, empty_B, max_timesteps, activation_threshold, n_iterations):
    ...
    start_neurons = set(meta_df[mask_A]['root_id'])
    print(start_neurons)
    end_neurons = set(meta_df[mask_B]['root_id'])
    if empty_B:
        end_neurons = set()
...
def create_activation_distance_df(metadata_df, cell_matrix, neuron_to_index):
    ...
    """
    activation_data = []
    metadata_df['root_id'] = metadata_df['root_id'].astype('string')

    for neuron_id in metadata_df['root_id']:
        ...
        activation_data.append({'root_id': neuron_id, 'distance': distance})
    result_df = pd.DataFrame(activation_data)
    result_df['root_id'] = result_df['root_id'].fillna('NaN').astype('string')
    result_df['root_id'] = result_df['root_id'].fillna('missing').astype('string')
