...
# Master folder
influence.master <- file.path(banc.influence.save.path,"banc_505/")
...
# Create .sql file
table_name <- "banc_505_influence"
dir.create(banc.influence.save.path, showWarnings = FALSE)
con <- DBI::dbConnect(RSQLite::SQLite(),
                      file.path(banc.influence.save.path,"influence_banc_505.sqlite"))
...
for(sg in seed.groups){
  ...
  for(csv in seed.files){
    ...
    data$influence_log <- log1p(data$influence)
    data$influence_norm <- as.numeric(data$influence)/n.seeds
    data$influence_norm_log <- log1p(data$influence_norm)
...
# remote_name <- "hms"
# local_file <- file.path(banc.influence.save.path,"influence_banc_505.sqlite")
