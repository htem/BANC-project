#' Sensory-to-effector adjusted-influence heatmaps (Fig. 2e)
#'
#' Pulls cached `query_influence()` output for each sensory super class onto
#' all effector neurons, groups effectors by `body_part_motor`, and renders
#' the row-normalised heatmap that is panel **Fig. 2e** in Bates, Phelps,
#' Kim, Yang et al., 2026. Statistical comparison (same-body-part vs.
#' different-body-part influence) is run with `wilcox.test(alternative =
#' "greater")`; the test statistic in the legend is the `W` for the matrix-
#' entry comparison (n_same = 14, n_different = 182).
#'
#' @section Reads:
#'   banc.meta, banc.edgelist.simple, cns.functions, paper.cols          (banc-startup.R)
#'   data/influence/.../<sensory_class>_influence.csv                    (per-seed cache)
#'
#' @section Writes:
#'   figures/figure_2/links/sensors_to_effectors_heatmap*.pdf            (Fig. 2e)
#'   figures/figure_2/links/supplement/sensors_to_effectors_*.pdf        (ED Fig. 4b–d)
#'   figures/figure_2/links/extra/...                                     (sensitivity sweeps, dark mode)
#'   figures/figure_2/links/*.txt                                         (Wilcoxon + Pearson summaries)
#'
#' @section Paper:
#'   Fig. 2e — sensory → effector body-part heatmap.
#'   ED Fig. 4b — per-body-part matched vs unmatched Wilcoxon boxplots.
#'   ED Fig. 4c — local / mid-range / long-range influence breakdown.
#'   ED Fig. 4d — effector body-part influence vs. cosine similarity.
#'   Methods §"Influence" (Eqs. 1–10, source-corrected Eq. 10 used here).
#'
#' @section Schema:
#'   `count_thresh` is the synapse-count floor passed to influencer (default 5,
#'   matches paper threshold "≥ 5 synapses"). Setting it to other values
#'   routes ALL outputs into figures/figure_2/links/extra/count_thresh_<N>/
#'   so the v888 figure-bound files are never overwritten by a sweep run.
#'
#' @section Used by:
#'   R/figures/figure-runner.R; outputs link directly into figure_2.ai and
#'   extended_data_figure_4.ai. Numbers cited in the doc come from
#'   R/text/numbers.R via the `figure_2/links/*.txt` companions.
#'
#' @section Reproduce:
#'   BANC_NCORES=1 Rscript R/figures/panels_sensory_motor.R
...
if (count_thresh == 5) {
  ...
  ct_dir <- sprintf("figures/figure_2/links/extra/count_thresh_%d", count_thresh)
  dir.create(ct_dir, showWarnings = FALSE, recursive = TRUE)
  banc.fig2.path <- ct_dir
  banc.fig2.supp.path <- ct_dir
  banc.fig2.extra.path <- ct_dir
}
...
# Global `count >= 3` filter removed 2026-04-09.
...
# rbind-in-loop. Also pre-index banc.meta by seed_02 once (avoids per-iter
# subset() on the full meta table).
...
.eff_id_set <- banc.eff.meta$id
# Uses banc_influence_loop() from banc-functions.R (PSOCK-parallel).
influence.sensors.db.orig <- banc_influence_loop(
  cts, "seed_02", "seed_02", .eff_id_set, ic = ic_banc
) %>%
  dplyr::left_join(.eff_target_lookup, by = "id") %>%
  dplyr::mutate(seed_class = "sensory")
...
seed.map <- c(
             ...
             pharynx_internal_taste_sensillum_gustatory_neuron = "pharynx",
             ...
             #labellum_orphan_neuron = "proboscis", 
             ...
             postocellar_bristle_neuron = "head", 
...
for(inf.metric in inf.metrics){
  ...
  clustering_result <- hclust_semisupervised(data = heatmap_matrix,
                                             groups = rownames(heatmap_matrix),
  ...
  # Paired — re-normalize minmax on the paired subset so each row's max
  # reflects the strongest paired source (not an unpaired source like retina)
  col.order.hm <- unique(c(intersect(paired.bps, colnames(reordered_matrix))))
  row.order.hm <- unique(c(intersect(paired.bps, rownames(reordered_matrix))))
  paired_submatrix <- reordered_matrix[row.order.hm, col.order.hm]
  if (grepl("minmax", inf.metric)) {
    paired_submatrix <- t(apply(paired_submatrix, 1, function(row) {
      mn <- min(row, na.rm = TRUE); mx <- max(row, na.rm = TRUE)
      if (mx == mn) rep(0, length(row)) else (row - mn) / (mx - mn)
    }))
  }
  pheatmap(
    paired_submatrix,
    color = scaled_heatmap_palette,
    breaks = if (grepl("minmax", inf.metric)) seq(0, 1, length.out = n_breaks) else scaled_heatmap_breaks,
    #annotation_row = cell_type_annotation,
    annotation_colors = annotation_colors,
    clustering_method = "ward.D2",
    cluster_rows = FALSE, 
    ...
    annotation_names_col = FALSE,
    annotation_names_row = FALSE,
    filename = file.path(banc.fig2.path, sprintf("%s_paired_sensors_to_effectors_heatmap.pdf",inf.metric))
  )
  ...
  paired.bps.avail <- intersect(paired.bps, intersect(rownames(reordered_matrix), colnames(reordered_matrix)))
  paired_matrix <- reordered_matrix[paired.bps.avail, paired.bps.avail]
...
print(p_facets)
ggplot2::ggsave(
  file.path(banc.fig2.extra.path, sprintf("%s_locality_and_unmatchable_faceted.pdf", inf.metric)),
...
pheatmap::pheatmap(
  ...
  annotation_names_col = FALSE,
  annotation_names_row = FALSE,
  filename = file.path(banc.fig2.extra.path, "sensory_sub_class_to_effector_by_body_part.pdf"),
  width = 24,
...
write.csv(stats_summary_optionA,
          file.path(banc.fig2.extra.path, "sensory_to_effector_celltype_level_stats.csv"),
          row.names = FALSE)
...
write.csv(stats_summary_optionB,
          file.path(banc.fig2.extra.path, "sensory_to_effector_bodypart_level_stats.csv"),
          row.names = FALSE)
...
if (!is.null(pairwise_details) && nrow(pairwise_details) > 0) {
  write.csv(pairwise_details,
            file.path(banc.fig2.extra.path, "sensory_to_effector_bodypart_pairwise_comparisons.csv"),
            row.names = FALSE)
}
...
writeLines(manuscript_text_optionB,
           file.path(banc.fig2.extra.path, "sensory_to_effector_bodypart_manuscript_summary.txt"))
...
write.csv(top2_test_summary,
          file.path(banc.fig2.extra.path, "top_2_sensory_body_parts_by_effector_body_parts_tests.csv"),
          row.names = FALSE)
writeLines(c("Top 2 Sensory Body Parts: Statistical Tests (Wilcoxon rank-sum test, two-sided, Holm-adjusted)",
             "",
             top2_test_text),
           file.path(banc.fig2.extra.path, "top_2_sensory_body_parts_by_effector_body_parts.txt"))
...
ggsave(
  filename = file.path(banc.fig2.extra.path, "top_2_sensory_body_parts_by_effector_body_parts.pdf"),
  plot = top2_plot,
  width = 12,
  height = 10,
  units = "in",
...
# Rbindlist refactor (2026-04-09): was rbind-in-loop accumulating to data.frame.
# The double loop structure is preserved because each (bp, ct) pair is written
# out with both the seed cell_type and the seed body_part — collapsing would
# lose that annotation. Pre-index banc.meta by seed_02 to avoid per-iter subset.
bps  <- banc.sens.meta %>%
...
.eff_target_lookup_ct <- banc.eff.meta %>%
  dplyr::distinct(id = root_id,
                  target = cell_type,
                  target_body_part = body_part_effector,
                  target_super_class = super_class)
# Flatten the body_part × cell_type nested loop into a single banc_influence_loop
# call, then annotate seed_body_part from a lookup table.
.bp_ct_map <- banc.sens.meta %>%
  dplyr::filter(!is.na(body_part_sensory), body_part_sensory != "unknown") %>%
  dplyr::distinct(cell_type, body_part_sensory)
.detailed_cts <- unique(na.omit(.bp_ct_map$cell_type))
influence.sensors.detailed.db.orig <- banc_influence_loop(
  .detailed_cts, "seed_02", "seed_02", .eff_id_set, ic = ic_banc
) %>%
  dplyr::left_join(.eff_target_lookup_ct, by = "id") %>%
  dplyr::left_join(.bp_ct_map, by = c("seed" = "cell_type")) %>%
  dplyr::rename(seed_body_part = body_part_sensory) %>%
  dplyr::mutate(seed_class = "sensory")
