#' Direct AN ↔ DN connectivity matrices by cluster (Fig. 3b, ED Fig. 8b)
#'
#' Computes cluster × cluster direct-connectivity heatmaps from the
#' proofread synaptic edge list, split by transmitter (all, glutamate,
#' GABA, acetylcholine). Each row is normalised to the post-cluster's
#' total input ("input fraction"). Produces Fig. 3b (AN/DN influence on
#' effectors) and the per-cluster cosine-similarity boxplots that compare
#' within-cell-type, within-cluster, and between-cluster influence (ED
#' Fig. 8b).
#'
#' Cell types excluded as "weird" — DNxl080, DNge079, DNg73, DNg65 — are
#' kept out of the clustering input because they have known morphological
#' asymmetries that distort their connectivity profile.
#'
#' @section Reads:
#'   banc.meta, banc.edgelist.simple, franken.meta, paper.cols
#'   data/banc_annotations/v888/banc_neck_functional_classes.csv                                  (cluster labels from UMAP)
#'
#' @section Writes:
#'   figures/figure_3/links/an_dn_direct_*.pdf                              (Fig. 3b)
#'   figures/figure_3/links/supplement/an_dn_cluster_similarity_*.pdf       (ED Fig. 8b)
#'   figures/figure_3/links/extra/                                          (per-NT variants)
#'   figures/figure_3/links/*.txt                                            (KW + pairwise Wilcoxon summaries)
#'
#' @section Paper:
#'   Fig. 3b — adjusted influence on effectors from DNs versus ANs.
#'   ED Fig. 8b — cosine-similarity boxplots within cell type / cluster / between.
#'   Methods §"Clustering influence by influence and connectivity"
#'   (the per-NT split is exploratory, not in the Methods text).
#'
#' @section Schema:
#'   `weird` set is a hard exclusion list — extend it in this script (not
#'   upstream) if a new asymmetric cell type needs to be dropped from the
#'   connectivity matrices.
#'
#' @section Reproduce:
#'   BANC_NCORES=1 Rscript R/figures/panels_an_dn_connectivity.R
...
for(connection.type in connection.types){
  ...
  if(connection.type=="all"){
    ...
    banc.an.dn.elist <- banc.edgelist.simple %>%
      ...
      dplyr::filter(!is.na(pre_cluster), 
                    !is.na(post_cluster), 
                    pre_neurotransmitter == connection.type) %>%
      dplyr::filter(pre %in% !!banc.an.dn.meta$root_id,
                    post %in% !!banc.an.dn.meta$root_id) %>%
      dplyr::distinct(pre_cluster, post_cluster, count, norm)  
  }
  ...
  # Choose color palette and breaks
  n_breaks <- 100
  ...
  if(connection.type=="all"){
    # Compute clustering on the *full square* matrix (reused for NT-specific plots)
  ...
  pheatmap::pheatmap(
    ...
    cellwidth = 8,
    cellheight = 8,
    filename = file.path(banc.fig3.extra.path, sprintf("%s_neck_cluster_to_neck_cluster_normalised_direct_connectivity.pdf",connection.type)),
...
elist_all_nt <- banc.edgelist.simple %>%
  #dplyr::filter(pre_neurotransmitter %in% connection.types[connection.types != "all"]) %>%
  dplyr::group_by(post_cluster) %>%
  dplyr::mutate(total = sum(count,na.rm = TRUE)) %>%
  dplyr::ungroup() %>%
  dplyr::filter(!is.na(pre_cluster), !is.na(post_cluster)) %>%
  dplyr::filter(pre %in% !!banc.an.dn.meta$root_id,
                post %in% !!banc.an.dn.meta$root_id) %>%
  dplyr::group_by(pre_cluster, post_cluster, pre_neurotransmitter) %>%
  dplyr::summarise(
...
# Order reciprocal by total bar height (sum of norm across NTs)
rec_pair_order <- elist_rec_plot %>%
  dplyr::group_by(pair_recip) %>%
  dplyr::summarise(total = sum(norm, na.rm = TRUE), .groups = "drop") %>%
  dplyr::arrange(total) %>%
  dplyr::pull(pair_recip)
elist_rec_plot$pair_recip <- factor(elist_rec_plot$pair_recip,
                                    levels = rec_pair_order)

# 1. Directed (stacked) plot
g.dir <- ggplot2::ggplot(elist_dir_plot,
                ggplot2::aes(x = pair, y = norm, fill = pre_neurotransmitter)) +
  ggplot2::geom_col(position = "stack") +
  ggplot2::coord_flip() +
  ggplot2::labs(
    x = "cluster pair",
    y = "proportion of postsynaptic input (by neurotransmitter)",
    fill = "presynaptic nt",
    title = "top directed connections by neurotransmitter"
  ) +
...
g.recip <- ggplot2::ggplot(elist_rec_plot,
                ggplot2::aes(x = pair_recip, y = norm, fill = pre_neurotransmitter)) +
  ggplot2::geom_col(position = "stack") +
  ggplot2::coord_flip() +
  ggplot2::labs(
    x = "cluster pair",
    y = "proportion of postsynaptic input (by neurotransmitter)",
    fill = "presynaptic nt",
    title = "top reciprocal connections by neurotransmitter"
  ) +
  ggplot2::theme_minimal() +
  ggplot2::theme(axis.text.y = ggplot2::element_text(size = 7)) +
  scale_fill_manual(values = paper.cols)
...
if (nrow(combo_top25) > 0) {
  ...
  message("No mixed super-cluster paths with ≥3 clusters found — skipping p_top25_mixed plot")
}
...
# Collapse pre_super_class categories that contribute < 1% of total inputs
# (across all post groups) into a single "other" bin coloured grey. This
# keeps the legend tractable and removes the long tail of tiny slices.
.pre_totals <- eff.up %>%
  dplyr::group_by(pre_super_class) %>%
  dplyr::summarise(total = sum(count, na.rm = TRUE), .groups = "drop") %>%
  dplyr::mutate(pct = total / sum(total))
.minor_pre <- .pre_totals %>%
  dplyr::filter(pct < 0.01) %>%
  dplyr::pull(pre_super_class) %>%
  as.character()
eff.up <- eff.up %>%
  dplyr::mutate(pre_super_class = ifelse(
    as.character(pre_super_class) %in% .minor_pre,
    "other",
    as.character(pre_super_class)
  )) %>%
  # Re-aggregate so all minor categories now flagged "other" collapse into a
  # single row per (post_super_class, post_region). Without this, ggplot draws
  # the white linewidth=0.2 stroke between every original-pre_super_class slice
  # even though they all share the grey "other" fill.
  dplyr::group_by(pre_super_class, post_super_class, post_region) %>%
...
order_pres <- eff.up %>%
  ...
  dplyr::pull(pre_super_class)
# Push "other" to the end so it sits at the right edge of the bar.
order_pres <- c(setdiff(order_pres, "other"), intersect(order_pres, "other"))
...
# Fill palette: known super_class colours from paper.cols, plus grey for "other".
.fill_palette <- paper.cols
if (!"other" %in% names(.fill_palette)) .fill_palette <- c(.fill_palette, other = "grey70")
...
p <- ggplot(eff.plot, aes(x = post_super_class, y = count, fill = pre_super_class)) +
  ...
  scale_fill_manual(values = .fill_palette, labels = ~ gsub("_", " ", .x)) +
...
# Save — bump height to 7 to give the 3-row bottom legend room (previously
# the bottom row of the legend was being clipped at height = 5.5).
ggsave(
  plot = p,
...
# Query influence data
chosen.seeds <- na.omit(unique(banc.an.dn.meta$seed_12))
influence.neck.df <- query_influence(
    levels = "seed_12", seeds = chosen.seeds,
    ids = banc.an.dn.meta$root_id, normalize = FALSE,
    ncores = 1L
  )

# Free the cached Python influence calculator now that we're done with it.
# It holds a sparse copy of the full BANC network in Python heap (multi-GB)
# and is the biggest reason this script OOM'd previously: we're about to
# build several copies of a 9.8M-row tibble inside calculate_influence_norms,
# and the Python heap was preventing it from fitting.
if (exists("banc.ic", envir = .GlobalEnv)) {
  rm(banc.ic, envir = .GlobalEnv)
  try(reticulate::py_run_string("import gc; gc.collect()"), silent = TRUE)
}
gc(verbose = FALSE)

# Lightweight cluster lookup (only columns needed downstream)
id_to_cluster <- banc.meta %>%
  dplyr::distinct(root_id, cluster) %>%
  dplyr::filter(!is.na(cluster))

# Map neuron IDs to cluster, aggregate, then normalize.
# Memory: build the joined df, drop the source, gc(), THEN call
# calculate_influence_norms. Avoids holding two ~9.8M-row tibbles at once.
influence.neck.cluster.df <- influence.neck.df %>%
  dplyr::mutate(seed = gsub(".*_", "", seed)) %>%
  dplyr::left_join(id_to_cluster, by = c("id" = "root_id")) %>%
  dplyr::rename(pre_cluster = cluster) %>%
  dplyr::left_join(id_to_cluster, by = c("seed" = "root_id")) %>%
  dplyr::rename(post_cluster = cluster) %>%
  dplyr::filter(!is.na(pre_cluster), !is.na(post_cluster)) %>%
  dplyr::mutate(seed = pre_cluster, target = post_cluster)
rm(influence.neck.df); gc(verbose = FALSE)
influence.neck.cluster.df <- calculate_influence_norms(influence.neck.cluster.df)
gc(verbose = FALSE)

# All by cluster — let the heatmap cluster rows/cols freely (no enforced order)
