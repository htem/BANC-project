#' Example neuroanatomy panels (Fig. 1c bottom, Fig. 4b inset, Methods exemplars)
#'
#' Renders the side-by-side BANC / FAFB / MANC / FANC neuroanatomy
#' figures used to illustrate the matching procedure (Methods §"Cell type
#' matching and annotation"). For each highlighted cell type, the script
#' reads neuron meshes via `bancr::banc_read_neuron_meshes()` and
#' equivalents in the cross-dataset wrappers, places them in a shared
#' BANC reference space using the rotation matrices baked into bancr,
#' and writes one PDF per cell type per source dataset.
#'
#' Current exemplars include LB1a (sensory ascending) and DVM1a-c
#' (indirect flight power motor neurons); additional cell types are
#' added in this script for ED Fig. 1 inset and Fig. 4b head-orienting
#' cluster blow-outs (DNa06, DNa11, etc.).
#'
#' @section Reads:
#'   banc.meta, franken.meta, banc_neuropil.surf, paper.cols
#'   neuron meshes via bancr::banc_read_neuron_meshes (GCS-backed)
#'
#' @section Writes:
#'   figures/figure_1/links/neuroanatomy/*.pdf                               (Fig. 1c bottom)
#'   figures/figure_4/links/neuroanatomy/*.pdf                               (Fig. 4b inset)
#'   figures/figure_4/links/supplement/*.pdf                                  (ED neuroanatomy examples)
#'
#' @section Paper:
#'   Fig. 1c — proofread / matched example pairs (LB1a, DVM1a-c).
#'   Fig. 4b — head-orienting cluster anatomy.
#'   Methods §"Cell type matching and annotation".
#'
#' @section Schema:
#'   `g.anat` ggplot template is the canonical anatomy frame for the
#'   paper — `theme_void()`, `coord_fixed()`, 8pt bold title, zero margin.
#'   Reuse this template if adding new anatomy panels.
#'
#' @section Notes:
#'   Always exempt from move rules: outputs live in
#'   `figures/figure_N/links/neuroanatomy/` and are referenced in-place
#'   by both main figure and ED .ai files.
#'
#' @section Reproduce:
#'   BANC_NCORES=1 Rscript R/figures/panels_example_neuroanatomies.R
...
# Save the plot
ggsave(plot = g.anat.banc,
       filename = file.path(banc.fig1.extra.path,"DNa02_banc.png"),
       width = 10, height = 10, dpi = 300)
...
ggsave(plot = g.anat.brain,
       filename = file.path(banc.fig1.extra.path,"dn_brain_graphic.png"),
       width = 10, height = 10, dpi = 300)
ggsave(plot = g.anat.vnc,
       filename = file.path(banc.fig1.extra.path,"dn_vnc_graphic.png"),
       width = 10, height = 10, dpi = 300)
...
ggsave(plot = g.anat.abrain,
       filename = file.path(banc.fig1.extra.path,"an_brain_graphic.png"),
       width = 10, height = 10, dpi = 300)
ggsave(plot = g.anat.avnc,
       filename = file.path(banc.fig1.extra.path,"an_vnc_graphic.png"),
       width = 10, height = 10, dpi = 300)
...
# Save the plot
ggsave(plot = g.anat.abrain,
       filename = file.path(banc.fig1.extra.path,"jrc2018_brain_graphic.png"),
       width = 10, height = 10, dpi = 300)
ggsave(plot = g.anat.avnc,
       filename = file.path(banc.fig1.extra.path,"jrc2018_vnc_graphic.png"),
       width = 10, height = 10, dpi = 300)
...
ggsave(plot = g.v.brain,
       filename = file.path(banc.fig4.extra.path, "brain_landing_vignette.png"),
       width = 10, height = 10, dpi = 300)
ggsave(plot = g.v.vnc,
       filename = file.path(banc.fig4.extra.path,"vnc_landing_vignette.png"),
       width = 10, height = 10, dpi = 300)
