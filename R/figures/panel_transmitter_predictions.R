###############################################################
## CROSS CONNECTOME ANALYSIS ON NEUROTRANSMITTER PREDICTIONS ##
###############################################################
...
region_levels <- c("optic_lobe","central_brain", "neck_connective","ventral_nerve_cord","CNS")
region_levels_disp <- c("optic lobe","central brain","neck connective","ventral nerve cord","CNS")
banc_pred <- banc.meta %>%
  dplyr::rowwise() %>%
  dplyr::mutate(
...
build_confusion_celltype <- function(banc_mode, other_mode, axis_levels = nt_levels) {
  ...
  # Label matrices: “X.X%\n(count)”
  labels_row <- matrix(
    paste0(sprintf("%.1f%%", as.numeric(mat_pct_row)), "\n(", as.integer(mat_counts), ")"),
    nrow = nrow(mat_counts), ncol = ncol(mat_counts),
    dimnames = dimnames(mat_counts)
  )
  labels_col <- matrix(
    paste0(sprintf("%.1f%%", as.numeric(mat_pct_col)), "\n(", as.integer(mat_counts), ")"),
    nrow = nrow(mat_counts), ncol = ncol(mat_counts),
    dimnames = dimnames(mat_counts)
  )
...
plot_heatmap_conf <- function(conf, outfile,
  ...
  main_title <- paste0(
    ...
    if (normalize == "row") "Row-normalized %" else "Column-normalized %", "  \n  ",
...
plot_heatmap_conf(conf_fafb_ct,
                  file.path(banc.fig1.supp.path, "banc_nt_confusion_celltype_BANC_vs_FAFB_rownorm.pdf"),
                  rows_name = "BANC", cols_name = "FAFB", normalize = "row"
)
plot_heatmap_conf(conf_manc_ct,
                  file.path(banc.fig1.supp.path, "banc_nt_confusion_celltype_BANC_vs_MANC_rownorm.pdf"),
                  rows_name = "BANC", cols_name = "MANC", normalize = "row"
)
plot_heatmap_conf(conf_hb_ct,
                  file.path(banc.fig1.supp.path, "banc_nt_confusion_celltype_BANC_vs_Hemibrain_rownorm.pdf"),
