#' panels_betweenness_layers.R — betweenness centrality (Fig. 3a, ED Fig. 5a)
#'
#' Brandes betweenness centrality of every BANC neuron, summarised by
#' super_class as violin + narrow-boxplot overlays. Two flavours are produced
#' (the paper uses both): all-to-all betweenness over every proofread (s, t)
#' pair, and sensory-to-effector betweenness restricted to sensory sources
#' and motor / visceral_circulatory / ascending_visceral_circulatory targets.
#' Side analyses (feedforward / backward layer scatter, betweenness vs layer
#' 2D-IQR plot, layer histograms) go to the supplement.
#'
#' @section Reads:
#'   * banc.meta                              via R/startup/banc-meta.R
#'   * banc.edgelist.simple                   via R/startup/banc-edgelist.R
#'   * data/banc_888_betweenness_afferent_to_efferent_v2.csv  (bancpipeline)
#'   * data/banc_888_betweenness_all_to_all_v2.csv            (bancpipeline)
#'   * data/feedforward/layers_banc_{888,626}.csv             (sjcabs FFW layers;
#'                                                             v626 fallback)
#'
#' @section Writes:
#'   * figures/figure_3/links/betweenness_violin_by_superclass.pdf
#'   * figures/figure_3/links/betweenness_violin_by_superclass_dunn_posthoc.txt
#'   * figures/figure_2/links/extra/betweenness_violin_by_class.pdf
#'   * figures/figure_2/links/supplement/betw_all_to_all_violin_by_superclass.pdf
#'   * figures/figure_3/links/supplement/betweenness_fwd_point_IQR_by_flow.pdf
#'   * Additional layer histograms + 2D-IQR scatter PDFs alongside .txt sidecars.
#'
#' @section Paper:
#'   * Fig. 3a    — sensory-to-effector betweenness by super_class.
#'   * ED Fig. 5a — all-to-all betweenness by super_class.
#'   * Methods §"Betweenness centrality" (Eqs. 11–12). Brandes
#'     unnormalised, unweighted, directed betweenness on the v2
#'     edgelist filtered to count >= 5; computed in Python / igraph by
#'     bancpipeline (`banc/metrics/`) and consumed here.
#'
#' @section Schema:
#'   * manuscript/print/dataverse/documentation/banc_888_betweenness.md
#'
#' @section Notes:
#'   * Runtime ~10 min on this machine (betweenness is precomputed; most
#'     of the cost is the violin + boxplot rendering).
#'
#' @section Reproduce: BANC_NCORES=1 Rscript R/figures/panels_betweenness_layers.R
...
# Output paths. Fig 3a lives in figure_3/links/; the all-to-all variant goes
# to figure_2/links/supplement/ for ED Fig 5a; exploratory plots land in
# extras/. The duplicated `banc.fig2.extra.path` assignment (supplement then
# extra) is intentional — the second assignment wins, so the script's later
# uses of `banc.fig2.extra.path` resolve to extra/. Kept as-is for backward
# compatibility with the historical save paths the .ai file links to.
banc.fig2.path       <- "figures/figure_2/links/"
banc.fig2.extra.path <- "figures/figure_2/links/supplement"  # overwritten below
banc.fig2.anat.path  <- "figures/figure_2/links/neuroanatomy"
banc.fig2.extra.path <- "figures/figure_2/links/extra"

# data — v2-flavored betweenness CSVs (produced by bancpipeline). The legacy
# unsuffixed / v626-fallback paths were removed 2026-04-18 when the pipeline
# adopted a versioned suffix; consumer default flipped from _v3 to _v2 on
# 2026-04-21.
betw_aff_file <- .banc_betweenness_afferent_csv
betw_all_file <- .banc_betweenness_all_csv
layer_file <- sprintf("data/feedforward/layers_banc_%d.csv", .version_num)
betw_id_col <- sprintf("root_%d", .version_num)
layer_id_col <- betw_id_col  # same versioned root column
if (!file.exists(layer_file)) {
  layer_file <- "data/feedforward/layers_banc_626.csv"
  layer_id_col <- "root_626"
  message(sprintf("Using v626 feedforward layer data (v%d not available) — joining on root_626", .version_num))
}

betweeness.data.raw <- read_csv(betw_aff_file, col_types = banc.col.types)
betweeness.data2.raw <- read_csv(betw_all_file, col_types = banc.col.types)
layer.data <- read_csv(layer_file, col_types = banc.col.types)

# Join betweenness data to meta via versioned root_id column
betweeness.data <- banc.meta %>%
  dplyr::left_join(betweeness.data.raw %>%
                     dplyr::rename(root_id = !!betw_id_col) %>%
                     ...
                     dplyr::distinct(root_id, betweenness), by = "root_id") %>%
  dplyr::left_join(layer.data %>%
                     dplyr::rename(layer_join_id = `Root ID`,
                                   fwd_layer = `FWD layer`,
                                   bwd_layer = `BWD layer`) %>%
                     dplyr::mutate(layer_join_id = as.character(layer_join_id),
                                   fwd_layer = as.numeric(fwd_layer),
                                   bwd_layer = as.numeric(bwd_layer)),
                   by = setNames("layer_join_id", layer_id_col)) %>%
  dplyr::left_join(betweeness.data2.raw %>%
                     dplyr::rename(root_id = !!betw_id_col) %>%
                     dplyr::mutate(root_id = as.character(root_id),
                                   betw_all = as.numeric(betweenness)) %>%
                     dplyr::distinct(root_id, betw_all), by = "root_id")
...
# Plot
plot(p_violin)
ggplot2::ggsave(file.path(banc.fig2.extra.path, "betweenness_violin_by_class.pdf"),
...
res_pairwise_class <- write_pairwise_wilcox(
  ...
  out_path = file.path(banc.fig2.extra.path, "betweenness_violin_by_class_pairwise_wilcox_summary.txt"),
  adjust_method = "holm",
  alpha = 0.05
)
...
bt2 <- bt %>%
  dplyr::group_by(super_class) %>%
  dplyr::mutate(.super_class_n = dplyr::n()) %>%
  dplyr::ungroup() %>%
  dplyr::filter(.super_class_n >= min_n) %>%
  dplyr::select(-.super_class_n) %>%
  dplyr::mutate(
...
plot(p_violin2)
ggplot2::ggsave(file.path(banc.fig3.path, "betweenness_violin_by_superclass.pdf"),
...
res_pairwise_betw <- write_pairwise_wilcox(
  ...
  comparisons = comparisons_betw,
  out_path = file.path(banc.fig3.path, "betweenness_pairwise_wilcox_summary.txt"),
  adjust_method = "holm",
  alpha = 0.05
)
...
res_dunn_betw <- write_dunn_posthoc(
  ...
  adjust_method = "holm",
  alpha = 0.05,
  append_to = file.path(banc.fig3.path, "betweenness_pairwise_wilcox_summary.txt")
)
...
# Filter out super_classes with fewer than 100 neurons (2026-04-10).
.sc_counts <- bt %>%
  dplyr::count(super_class) %>%
  dplyr::filter(n >= 100)
bt.all <- bt %>%
  dplyr::filter(super_class %in% .sc_counts$super_class) %>%
  dplyr::mutate(
    super_class = forcats::fct_relevel(super_class, intersect(super_order, .sc_counts$super_class)),
    super_class = factor(super_class)
  ) %>%
  dplyr::mutate(betw_all = pmax(betw_all, eps))
message(sprintf("betw_all violin: keeping %d super_classes with >= 100 neurons (dropped %d)",
                nrow(.sc_counts), length(super_order) - nrow(.sc_counts)))
...
p_violin_all <- ggplot2::ggplot(bt.all, ggplot2::aes(x = super_class, y = betw_all, fill = super_class)) +
  ...
  ggplot2::labs(
    x = NULL,
    y = paste0("betw_all_to_all (log10, ε=", signif(eps, 2), " added to zeros)"),
    fill = "super_class"
  ) +
  ggplot2::theme_minimal() +
  ggplot2::theme(
...
# Plot and save!
plot(p_violin_all)
ggplot2::ggsave(file.path(banc.fig2.supp.path, "betw_all_to_all_violin_by_superclass.pdf"),
...
res_pairwise_betw_all <- write_pairwise_wilcox(
  ...
  group_col = super_class,
  comparisons = comparisons_betw_all,
  out_path = file.path(banc.fig2.supp.path, "betw_all_to_all_pairwise_wilcox_summary.txt"),
  adjust_method = "holm",
  alpha = 0.05
)
...
res_dunn_betw_all <- write_dunn_posthoc(
  ...
  adjust_method = "holm",
  alpha = 0.05,
  append_to = file.path(banc.fig2.supp.path, "betw_all_to_all_pairwise_wilcox_summary.txt")
)
...
# `kw_dunn_summary()` lives in R/startup/banc-functions.R
# (hoisted 2026-05-21).
...
writeLines(dunn_out, file.path(banc.fig3.path, "betweenness_violin_by_superclass_dunn_posthoc.txt"))
cat("Saved:", file.path(banc.fig3.path, "betweenness_violin_by_superclass_dunn_posthoc.txt"), "\n")
...
# Plot and save!
plot(p_violin_all)
ggplot2::ggsave(file.path(banc.fig2.extra.path, "fwd_layereenness_violin_by_cns_network.pdf"),
...
# rbind-in-loop. Also hoist the eff target lookup out of the per-iter join.
...
# Pre-compute fwd_layer_round once (was recomputed per iteration)
.btw_fwd <- betweeness.data %>%
  dplyr::mutate(fwd_layer_round = round_any(fwd_layer, 100)) %>%
  dplyr::filter(!grepl("sensory", super_class))

banc.efferent_influence_list <- vector("list", length(layers))
for(li in seq_along(layers)){
  lay <- layers[[li]]
  banc.ids <- .btw_fwd %>%
    dplyr::filter(fwd_layer_round == lay) %>%
    dplyr::pull(root_id)
  if (length(banc.ids) == 0) next
  try({
    ...
    banc.efferent_influence_list[[li]] <- efferent_influence.id
  })
}
...
# Get influence results for BANC (betweenness-round variant).
# Rbindlist refactor (2026-04-09). banc.eff.meta / .eff_target_lookup are
# unchanged from above.
efferent_influence.signed <- data.frame()
layers <- sort(unique(round_any(log10(1+betweeness.data$betw_all),0.5)))

# Pre-compute the betweenness_round column once (was recomputed per iter).
.btw_ranked <- betweeness.data %>%
  dplyr::mutate(betweenness_round = round_any(log10(1 + betw_all), 0.5)) %>%
  dplyr::filter(betweenness != 0, !grepl("sensory", super_class))

banc.efferent_influence_list <- vector("list", length(layers))
for(li in seq_along(layers)){
  lay <- layers[[li]]
  banc.ids <- .btw_ranked %>%
    dplyr::filter(betweenness_round == lay) %>%
    dplyr::pull(root_id)
  if (length(banc.ids) == 0) next
  try({
    efferent_influence.id <- calculate_influence_py(ic_banc, banc.ids) %>%
...
# Get influence results for BANC (layers → layers).
# Rbindlist refactor (2026-04-09). Reuses the pre-computed .btw_fwd (non-sensory,
# fwd_layer_round) from the first block. The fwd_layer target lookup is also
# hoisted out of the per-iter left_join.
banc_influence.signed <- data.frame()
layers <- sort(unique(round_any(betweeness.data$fwd_layer, 100)))
.fwd_layer_target <- betweeness.data %>%
  dplyr::mutate(target = plyr::round_any(fwd_layer, 100)) %>%
  dplyr::distinct(id = root_id, target)

banc.banc_influence_list <- vector("list", length(layers))
for(li in seq_along(layers)){
  lay <- layers[[li]]
  banc.ids <- .btw_fwd %>%
    dplyr::filter(fwd_layer_round == lay) %>%
  ...
  try({
    banc_influence.id <- calculate_influence_py(ic_banc, banc.ids) %>%
      dplyr::left_join(.fwd_layer_target, by = "id")
    ...
    banc.banc_influence_list[[li]] <- banc_influence.id
  })
}
banc.banc_influence <- as.data.frame(
  data.table::rbindlist(banc.banc_influence_list, fill = TRUE)
) %>%
  dplyr::mutate(influence_original = `Influence_score_(unsigned)`)
...
eff.out.key.plot <- banc_plot_key_features(
  influence.meta = {
    # Build root_id → fwd_layer lookup via layer_id_col (root_626 for fallback)
    .layer_lookup <- layer.data %>%
      dplyr::distinct(layer_join_id = `Root ID`, fwd_layer = as.numeric(`FWD layer`)) %>%
      dplyr::mutate(layer_join_id = as.character(layer_join_id))
    .id_bridge <- banc.meta %>%
      dplyr::select(root_id, dplyr::all_of(layer_id_col)) %>%
      dplyr::distinct() %>%
      dplyr::left_join(.layer_lookup, by = setNames("layer_join_id", layer_id_col))
    banc.edgelist.simple %>%
      dplyr::left_join(.id_bridge %>% dplyr::select(pre = root_id, pre_fwd_layer = fwd_layer), by = "pre") %>%
      dplyr::left_join(.id_bridge %>% dplyr::select(post = root_id, post_fwd_layer = fwd_layer), by = "post")
  } %>%
